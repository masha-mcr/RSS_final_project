...
def train(data_path: Path, model_alg: str, save_model_path: Path, random_state: int, use_scaler: bool, tuning: str, test_split_ratio: float)-> None:
    click.echo('Loading and preprocessing data.')
    df = preprocess(get_dataset(data_path))
    X_train, X_test, y_train, y_test  = data_train_test_split(df, test_split_ratio, random_state) 
    ...
    if tuning == 'default':
        ...
        best_pipeline = run_nestedCV(X_train, y_train, X_test, y_test, 
                                use_scaler, numeric_cols, model_alg, random_state, tuning)
    else: 
...
def run_default(X_train: pd.DataFrame, y_train: pd.Series, 
                X_test: pd.DataFrame, y_test: pd.Series, use_scaler: bool, 
                    numeric_cols: list, model_alg: str, random_state: int)-> Pipeline:
...
def run_nestedCV(X_train: pd.DataFrame, y_train: pd.Series, 
                X_test: pd.DataFrame, y_test: pd.Series, use_scaler: bool, 
                    numeric_cols: list, model_alg: str, random_state: int, tuning: str)-> Pipeline:
    with mlflow.start_run():
        pipeline = create_pipeline(use_scaler, numeric_cols, model_alg, random_state)
        ...
        for train_idx, test_idx in cv_outer.split(X_train):
            ...
            if tuning == 'grid':
                search = GridSearchCV(pipeline, param_grid, scoring='accuracy', cv=cv_inner, refit=True, n_jobs=-1, error_score=0)
                click.echo('Starting GridSearchCV on outer fold.')
            elif tuning == 'random':
                search = RandomizedSearchCV(pipeline, param_grid, scoring='accuracy', cv=cv_inner, refit=True, n_jobs=-1,error_score=0)
